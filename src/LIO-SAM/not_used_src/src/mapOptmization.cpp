...
// === INSERT START: TF tools and lidar→base cache ===
    // TF2 buffer/listener to read static base_link ↔ lidar extrinsics from your URDF/static publisher
    std::shared_ptr<tf2_ros::Buffer> tf_buffer_;
    std::shared_ptr<tf2_ros::TransformListener> tf_listener_;

    // Cached transform (lidar frame expressed in base_link) and a flag to know when it’s ready
    tf2::Transform T_lidar_to_base_;
    bool T_lidar_to_base_ready_ = false;

    // Helper: ensure we have the static TF; called before we compose odom→base from odom→lidar
    void ensureStaticLidarToBaseTF()
    {
        if (T_lidar_to_base_ready_) return;

        try {
            // We look up base_link <- lidarFrame (same names you already load from YAML)
            auto st = tf_buffer_->lookupTransform(baselinkFrame, lidarFrame, tf2::TimePointZero);
            tf2::Transform T_base_lidar;
            tf2::fromMsg(st.transform, T_base_lidar);

            // Cache lidar→base (inverse)
            T_lidar_to_base_ = T_base_lidar.inverse();
            T_lidar_to_base_ready_ = true;
...
// === INSERT END ===
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        pubPath = create_publisher<nav_msgs::msg::Path>("lio_sam/mapping/path", 1);
            // ---  added TF broadcaster: REPLACE THIS LINE ---
        // br = std::make_unique<tf2_ros::TransformBroadcaster>(this);
        // with one of these (pick the one your ROS 2 version supports):
        br = std::make_unique<tf2_ros::TransformBroadcaster>(
            this->get_node_base_interface(),
            this->get_node_clock_interface(),
            this->get_node_logging_interface(),
            this->get_node_topics_interface());   // common pattern
        // OR:
        // br = std::make_unique<tf2_ros::TransformBroadcaster>(this->get_node_base_interface(),
        //                                                      this->get_clock());

        // --- TF listener/buffer: ADD these right after 'br' ---
    ...
    void publishOdometry()
    {
        ...
        laserOdometryROS.child_frame_id = baselinkFrame;   // from YAML
        ...
        //added for tf 
        ensureStaticLidarToBaseTF();
        // If we don't have the static TF yet, fall back to using lidar frame directly (not ideal)
        tf2::Transform t_odom_to_base = T_lidar_to_base_ready_
            ? (t_odom_to_lidar * T_lidar_to_base_)
            : t_odom_to_lidar;

        // Overwrite Odometry pose with base_link pose
        geometry_msgs::msg::Pose base_pose_msg;
        tf2::toMsg(t_odom_to_base, base_pose_msg);
        laserOdometryROS.pose.pose = base_pose_msg;


        geometry_msgs::msg::TransformStamped trans_odom_to_base;
        trans_odom_to_base.header.stamp = timeLaserInfoStamp;     // rclcpp::Time you already have
        trans_odom_to_base.header.frame_id = odometryFrame;       // e.g., "odom"
        trans_odom_to_base.child_frame_id  = baselinkFrame;       // e.g., "base_link"
        trans_odom_to_base.transform = tf2::toMsg(t_odom_to_base);
        br->sendTransform(trans_odom_to_base);

        // (Optional legacy TF for consumers expecting odom->lidar)
        #if 0
        geometry_msgs::msg::TransformStamped trans_odom_to_lidar;
        trans_odom_to_lidar.header.stamp = timeLaserInfoStamp;
        trans_odom_to_lidar.header.frame_id = odometryFrame;
        trans_odom_to_lidar.child_frame_id  = lidarFrame;         // from YAML
        trans_odom_to_lidar.transform = tf2::toMsg(t_odom_to_lidar);
        br->sendTransform(trans_odom_to_lidar);
        #endif
        // (Optional) Also publish odom->lidar for backwards compatibility
        // geometry_msgs::msg::TransformStamped trans_odom_to_lidar;
        // tf2::convert(tf2::Stamped<tf2::Transform>(t_odom_to_lidar, time_point, odometryFrame), trans_odom_to_lidar);
        ...
        if (lastIncreOdomPubFlag == false)
        {
            ...
            // 1) Build the incremental odom->lidar transform from x,y,z,roll,pitch,yaw
            tf2::Quaternion q_inc;
            q_inc.setRPY(roll, pitch, yaw);
            tf2::Transform t_odom_to_lidar_inc(q_inc, tf2::Vector3(x, y, z));

            // 2) Ensure we know the static lidar->base_link extrinsic (from URDF/static TF)
            ensureStaticLidarToBaseTF();

            // 3) Convert to odom->base_link using the extrinsic
            tf2::Transform t_odom_to_base_inc = T_lidar_to_base_ready_
                ? (t_odom_to_lidar_inc * T_lidar_to_base_)
                : t_odom_to_lidar_inc;  // fallback if static TF not available yet

            // 4) Stuff that pose into the odometry message
            geometry_msgs::msg::Pose base_inc_pose;
            tf2::toMsg(t_odom_to_base_inc, base_inc_pose);
            laserOdomIncremental.pose.pose = base_inc_pose;
